...
test("Founder redemption requests publication in the same server action",()=>{const body=actions.slice(actions.indexOf("export async function redeemFounderAccess"),actions.indexOf("export async function joinTrainerWaitlist"));assert.ok(body.indexOf("redeem_my_access_code")<body.indexOf("request_my_site_publication"));assert.match(onboarding,/if\(canPublish&&!accessState\?\.founder_access_active\)return"ready"/);});

test("waitlist has a dedicated factual success stage with safe navigation",()=>{assert.match(onboarding,/"waitlist_success"/);assert.match(onboarding,/Você entrou na lista de espera/);assert.match(onboarding,/rascunho privado/);assert.match(onboarding,/Founder Access e Billing não foram ativados/);assert.match(onboarding,/setEditingStage\("access"\)/);assert.match(onboarding,/formAction=\{logout\}|action=\{logout\}/);assert.match(migration,/waitlist_email/);assert.match(migration,/waitlist_whatsapp/);});

...
const wordmark = read("../../src/components/auth/PperfilAnimatedWordmark.tsx");
...
test("Welcome keeps the approved cinematic PPerfil composition", () => {
  ...
  assert.match(entry, /<PperfilAnimatedWordmark id="cheipi-welcome-title" \/>/);
  assert.match(wordmark, /aria-label="PPerfil"/);
  assert.match(wordmark, /pperfil-wordmark-static\.png/);
  assert.doesNotMatch(wordmark, />PPERFIL</);
  assert.match(css, /pperfil-wordmark-mask\.png/);
  assert.match(css, /@keyframes pperfil-wordmark-reveal/);
  assert.match(css, /@keyframes pperfil-wordmark-breathe/);
  assert.match(css, /@keyframes pperfil-wordmark-sheen/);

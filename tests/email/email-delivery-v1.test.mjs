...
test("invitation persists before email attempt and delivery state stays factual",()=>{const actionBody=actions.slice(actions.indexOf("export async function inviteStudentAction"),actions.indexOf("export async function resendInvitationAction"));assert.ok(actionBody.indexOf("createStudentInvitation(email)")<actionBody.indexOf("deliverInvitation(invite,email)"));assert.match(actions,/emailDelivery:sent\?"email_sent":"email_failed"/);assert.match(actions,/invitationCreated:true,emailDelivery:"email_failed"/);assert.match(actions,/Convite criado, mas o e-mail não pôde ser enviado/);assert.match(form,/Link do convite/)});

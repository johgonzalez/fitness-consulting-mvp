...
test("invitation persists before email attempt and delivery state stays factual",()=>{const actionBody=actions.slice(actions.indexOf("export async function inviteStudentAction"));assert.ok(actionBody.indexOf("createNamedInvitation(name,email)")<actionBody.indexOf("sendTransactionalEmail"));assert.match(actions,/invitationCreated:true,emailDelivery:"email_sent"/);assert.match(actions,/invitationCreated:true,emailDelivery:"email_failed"/);assert.match(actions,/Convite criado, mas o e-mail não pôde ser enviado/);assert.match(form,/Link do convite/)});

test("duplicate delivery and duplicate invitation creation are constrained",()=>{assert.match(provider,/"Idempotency-Key": message\.idempotencyKey/);assert.match(template,/student-invitation\/\$\{input\.invitationId\}/);assert.match(invitationMigration,/student_invitations_one_pending_target_idx[\s\S]*where status = 'pending'/i)});

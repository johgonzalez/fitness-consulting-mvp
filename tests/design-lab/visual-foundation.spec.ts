...
async function theme(page:Page,value:"light"|"dark") { const main=page.locator("main"); if(await main.getAttribute("data-theme")!==value) await page.getByRole("button",{name:value==="dark"?"Ativar tema escuro":"Ativar tema claro"}).click(); await expect(main).toHaveAttribute("data-theme",value); }
...
test("Gate 1B starts unbiased, requires all decisions and persists its draft", async ({page}) => {
  ...
  await expect(page.getByRole("heading",{level:1})).toContainText("Escolha o sistema");
  await expect(page.getByRole("radio")).toHaveCount(16);
  await expect(page.getByRole("radio",{checked:true})).toHaveCount(0);
  await expect(page.getByRole("button",{name:"Aprovar Gate 1B"})).toBeDisabled();
...
test("combined preview keeps one system across Login, Dashboard and Workout", async ({page}) => {
  await page.goto("/design-lab/v1"); await chooseAll(page);
  ...
  await page.getByRole("tab",{name:"Workout"}).click(); await expect(preview.getByText("Agachamento livre")).toBeVisible();
});
...
test("approval confirmation locks controls and reopening requires its own confirmation", async ({page}) => {
  ...
  await expect(page.getByText("Gate 1B aprovado. Fundação visual registrada.")).toBeVisible(); await expect(page.getByRole("radio").first()).toBeDisabled();
  const reopen=page.getByRole("button",{name:"Reabrir decisões"}); await reopen.click(); const reopenDialog=page.getByRole("dialog",{name:"Reabrir decisões?"}); await expect(reopenDialog).toBeVisible();
  await reopenDialog.getByRole("button",{name:"Reabrir Gate 1B"}).click(); await expect(page.getByText(/Gate 1B reaberto/)).toBeVisible(); await expect(page.getByRole("radio").first()).toBeEnabled();
});

test("keyboard selection, dialogs and serious accessibility violations", async ({page}) => {
  await page.setViewportSize({width:390,height:844}); await page.goto("/design-lab/v1");
...
for (const viewport of viewports) for (const mode of ["light","dark"] as const) test(`${viewport.width}px ${mode}: responsive system`,async({page})=>{await page.setViewportSize(viewport);await page.goto("/design-lab/v1");await theme(page,mode);await noOverflow(page);await expect(page.getByRole("heading",{name:"Territórios visuais"})).toBeVisible();});

test("200% text and keyboard-height mobile states reflow",async({page})=>{await page.setViewportSize({width:390,height:500});await page.goto("/design-lab/v1#fields");await page.evaluate(()=>document.documentElement.style.fontSize="200%");await noOverflow(page);const input=page.getByLabel("Hairline vazio");await input.focus();await expect(input).toBeInViewport();});
test("reduced motion removes sustained animation",async({page})=>{await page.emulateMedia({reducedMotion:"reduce"});await page.goto("/design-lab/v1#motion");expect(await page.getByText("Série concluída").evaluate(el=>getComputedStyle(el).animationDuration)).toBe("0s");});

test("required Gate 1B evidence screenshots",async({page})=>{
  await page.setViewportSize({width:390,height:844});await page.goto("/design-lab/v1");await chooseAll(page);const dir=path.join("test-results","design-lab","screenshots");
  ...
  for(const [tab,file] of [["Login","gate-1b-combination-login-390.png"],["Dashboard","gate-1b-combination-dashboard-390.png"],["Workout","gate-1b-combination-workout-390.png"]] as const){await page.getByRole("tab",{name:tab}).click();await page.locator("#preview").screenshot({path:path.join(dir,file)});}
...
test("unlabeled QA mode preserves accessible option names and visual signatures",async({page})=>{await page.goto("/design-lab/v1?qa=unlabeled");await expect(page.locator("main")).toHaveAttribute("data-unlabeled","true");await expect(page.getByRole("radio")).toHaveCount(16);await expect(page.locator('[data-qa="territories"] [data-territory="A"]')).toHaveCSS("background-color","rgb(255, 255, 255)");await expect(page.locator('[data-qa="territories"] [data-territory="C"]')).toHaveCSS("background-color","rgb(29, 36, 32)");});

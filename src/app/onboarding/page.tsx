...
export default async function OnboardingPage({searchParams}:{searchParams:Promise<OnboardingSearchParams>}){
  ...
  const user=await requireUser();const profile=await findOwnerProfile();const supabase=await createClient();const draftResult=await supabase.rpc("get_my_onboarding_draft");const draft=(draftResult.data as OnboardingDraft|null)??null;let billing:BillingSummary|null=null;let accessState:AccessState|null=null;let canPublish=false;let studentActivation:"none"|"pending"|"active"="none";

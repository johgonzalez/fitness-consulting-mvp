...
type AccessState={founder_access_active?:boolean;waitlist_joined?:boolean};
...
export default async function OnboardingPage({searchParams}:{searchParams:Promise<OnboardingSearchParams>}){
  ...
  const user=await requireUser();const profile=await findOwnerProfile();let draft:OnboardingDraft|null=null;let billing:BillingSummary|null=null;let accessState:AccessState|null=null;let canPublish=false;let hasStudentActivity=false;
  if(!profile){const supabase=await createClient();const{data}=await supabase.rpc("get_my_onboarding_draft");draft=data as OnboardingDraft|null;}else{const supabase=await createClient();const[{data},access,entitlements,students]=await Promise.all([supabase.rpc("get_my_billing_summary"),supabase.rpc("get_my_access_state"),findOwnerEntitlements(),getStudentsWorkspace().catch(()=>null)]);billing=data as BillingSummary|null;accessState=access.data as AccessState|null;canPublish=entitlements?.can_publish_site===true;hasStudentActivity=Boolean(students&&(students.students.length||students.invitations.length));}
  const origin=process.env.NEXT_PUBLIC_SITE_URL?.replace(/\/$/,"")??"";
  return <main className="onboarding-page"><section className="onboarding-panel"><Link href="/" className="saas-brand">PPerfil</Link><OnboardingForm key={query.updated??"current"} draft={draft} profile={profile} billing={billing} accessState={accessState} accountEmail={user.email??""} canPublish={canPublish} hasStudentActivity={hasStudentActivity} publicUrl={profile?`${origin}/p/${profile.slug}`:null}/></section></main>;
}

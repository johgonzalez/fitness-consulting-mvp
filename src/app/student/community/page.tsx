...
export default async function StudentCommunityPage({ searchParams }: { searchParams: Promise<{ community?: string; filter?: string; shareWorkout?: string; beforeAt?: string; beforeId?: string }> }) {
  ...
  return <CommunityFeed workspace={await getCommunityWorkspace("student", query.community, filter, query.beforeAt, query.beforeId)} audience="student" shareWorkoutExecutionId={query.shareWorkout} studentPhotosEnabled={process.env.COMMUNITY_STUDENT_PHOTO_POSTS_ENABLED === "true"} />;
}

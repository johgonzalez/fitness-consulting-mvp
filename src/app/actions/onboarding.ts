...
export async function saveOnboardingIdentity(_state:OnboardingActionState,form:FormData):Promise<OnboardingActionState>{
  ...
  const file=form.get("image");if(file instanceof File&&file.size>0){try{const image=await normalizeProfileImage(file);const path=`${ctx.user.id}/onboarding/profile/${crypto.randomUUID()}.${image.extension}`;const upload=await ctx.supabase.storage.from("trainer-public-media").upload(path,image.data,{contentType:image.contentType,cacheControl:"31536000",upsert:false});if(upload.error)return{message:"Não foi possível enviar sua foto agora. Tente novamente."};profileImageUrl=ctx.supabase.storage.from("trainer-public-media").getPublicUrl(path).data.publicUrl;}catch(error){return{message:profileImageMessage(error)}}}

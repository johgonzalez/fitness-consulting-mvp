...
export async function saveOnboardingIdentity(_state:OnboardingActionState,form:FormData):Promise<OnboardingActionState>{
  ...
  const{error}=await ctx.supabase.rpc("save_my_onboarding_identity",{p_display_name:displayName,p_professional_name:professionalName||null,p_profile_image_url:profileImageUrl});if(error)return{message:"Não foi possível salvar sua identidade."};redirectToAuthoritativeOnboarding();
}

...
export const COMMUNITY_IMAGE_MAX_EDGE = 2048;
...
export async function hardenCommunityImage(file: File, maxEdge = COMMUNITY_IMAGE_MAX_EDGE) {
  ...
  const { data, info } = await pipeline.rotate().resize({ width: maxEdge, height: maxEdge, fit: "inside", withoutEnlargement: true }).webp({ quality: 82, effort: 4 }).toBuffer({ resolveWithObject: true });

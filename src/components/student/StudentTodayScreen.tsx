...
export function StudentTodayScreen({ workspace, demoState = "active" }: { workspace: StudentTodayWorkspace; demoState?: TodayDemoState }) {
  ...
  return <div className="pp-student-page pc-student-today">
    ...
    {workspace.challenges.length ? <section className="pc-today-challenges" aria-labelledby="today-challenges-title"><header><span>Comunidade</span><h2 id="today-challenges-title">Desafios aceitos</h2></header>{workspace.challenges.map((challenge) => <article key={challenge.id}><div><strong>{challenge.title}</strong><span>{challenge.groupName}{challenge.durationMinutes ? ` · ${challenge.durationMinutes} min` : ""}</span></div>{challenge.acceptanceStatus === "COMPLETED" ? <span><Check aria-hidden="true"/>Concluído</span> : challenge.workoutSessionId ? <Link href={`/student/workouts/${challenge.workoutSessionId}`}>Ver treino<ArrowRight aria-hidden="true"/></Link> : <span><Check aria-hidden="true"/>Aceito</span>}</article>)}</section> : null}

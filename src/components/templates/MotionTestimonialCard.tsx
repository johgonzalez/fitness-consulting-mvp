...
type MotionTestimonial = TrainerSiteData["testimonials"][number];
...
export function MotionTestimonialCard({ testimonial }: { testimonial: MotionTestimonial }) {
  return (
    <blockquote className={styles.testimonialCard} data-motion="text">
      <header className={styles.testimonialHeader}>
        <span className={styles.testimonialAvatar}>
          {testimonial.image ? (
            <Image src={testimonial.image} alt="" fill sizes="46px" unoptimized />
          ) : (
        ...
        <span className={styles.testimonialIcon} aria-hidden="true"><MessageCircle /></span>
      </header>

...
function QuestionField({ question, value, onChange, labelledBy, describedBy }: {
  ...
  const aria = { "aria-labelledby": labelledBy, "aria-describedby": describedBy };
  if (question.type === "SHORT_TEXT" || question.type === "LONG_TEXT") {
    const props = { ...aria, value: typeof value === "string" ? value : "", onChange: (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => onChange(event.target.value), maxLength: 5000, placeholder: "Escreva sua resposta…" };
    return question.type === "LONG_TEXT" ? <textarea {...props} rows={7} /> : <input {...props} type="text" />;
  }
  if (question.type === "NUMBER") return <div className="pp-student-number"><input {...aria} type="number" inputMode="decimal" value={typeof value === "number" ? value : ""} onChange={(event) => onChange(Number(event.target.value))} placeholder="0" /></div>;
  if (question.type === "DATE") return <input {...aria} type="date" value={typeof value === "string" ? value : ""} onChange={(event) => onChange(event.target.value)} />;
  if (question.type === "BOOLEAN") return <div className="pp-student-options pp-student-options--binary" role="group" {...aria}>{[{ value: true, label: "Sim" }, { value: false, label: "Não" }].map((option) => <button key={String(option.value)} type="button" aria-pressed={value === option.value} onClick={() => onChange(option.value)}><span>{option.label}</span>{value === option.value ? <Check aria-hidden="true" /> : null}</button>)}</div>;
  if (question.type === "SINGLE_CHOICE") return <div className="pp-student-options" role="group" {...aria}>{question.options.map((option) => <button key={option.value} type="button" aria-pressed={value === option.value} onClick={() => onChange(option.value)}><span>{localText(option.label)}</span>{value === option.value ? <Check aria-hidden="true" /> : null}</button>)}</div>;
  if (question.type === "MULTI_CHOICE") {
    const selected = Array.isArray(value) ? value : [];
    return <div className="pp-student-options" role="group" {...aria}>{question.options.map((option) => <button key={option.value} type="button" aria-pressed={selected.includes(option.value)} onClick={() => onChange(selected.includes(option.value) ? selected.filter((entry) => entry !== option.value) : [...selected, option.value])}><span>{localText(option.label)}</span>{selected.includes(option.value) ? <Check aria-hidden="true" /> : null}</button>)}</div>;
  }
  if (question.type === "SCALE") return <div className="pp-scale-field" role="group" {...aria}><div>{Array.from({ length: question.scale.max - question.scale.min + 1 }, (_, index) => question.scale.min + index).map((number) => <button type="button" key={number} aria-pressed={value === number} onClick={() => onChange(number)}>{number}</button>)}</div><span><small>Menor</small><small>Maior</small></span></div>;
...
export function StudentAssessmentExperience({ assessment, trainer, demoMode }: { assessment: AssessmentDetail; trainer: { name: string; imageUrl: string | null; credential: string | null }; demoMode: boolean }) {
  ...
  if (view === "intro") return <section className="pp-student-assessment pp-student-assessment--intro" aria-labelledby="assessment-intro-title">
    <div className="pp-student-card">
      ...
      <div className="pp-student-intro-copy"><p>Avaliação</p><h1 id="assessment-intro-title">{assessment.title}</h1><span>{purposeFromMetadata(assessment.templateSchema.metadata)}</span></div>
  ...
  if (view === "success") return <section className="pp-student-assessment pp-student-assessment--success" aria-labelledby="assessment-success-title"><div className="pp-student-card"><span className="pp-success-mark"><Check aria-hidden="true" /></span><p>Respostas enviadas</p><h1 id="assessment-success-title">Avaliação concluída com sucesso</h1><span>Seu Personal já pode revisar suas respostas e preparar uma devolutiva.</span>{demoMode ? <small className="pp-demo-note">Simulação local: nenhum dado foi enviado ao Supabase.</small> : null}<button className="pp-button pp-button--secondary pp-student-primary" onClick={() => setView("readonly")}>Ver minhas respostas</button></div></section>;

  if (view === "readonly") return <section className="pp-student-assessment pp-student-assessment--readonly" aria-labelledby="assessment-readonly-title"><div className="pp-student-card">
    <header><div><p>{assessment.status === "COMPLETED" ? "Avaliação concluída" : assessment.status === "IN_REVIEW" ? "Em revisão pelo Personal" : "Respostas enviadas"}</p><h1 id="assessment-readonly-title">{assessment.title}</h1></div><AssessmentStatusBadge status={assessment.status === "SENT" ? "ANSWERED" : assessment.status} /></header>
  ...
  if (view === "review") return <section className="pp-student-assessment pp-student-assessment--review" aria-labelledby="assessment-review-title"><div className="pp-student-card">
    ...
    <div className="pp-student-review-heading"><p>Revisão final</p><h1 id="assessment-review-title">Revise e envie</h1><span>Confira suas respostas. Depois do envio, elas ficam somente para leitura.</span></div>
  ...
  const descriptionId = question.description ? "assessment-question-description" : undefined;
  const errorId = error ? "assessment-question-error" : undefined;
  return <section className="pp-student-assessment pp-student-assessment--question" aria-labelledby="assessment-question-title"><div className="pp-student-card">
    ...
    <div className="pp-student-progress" role="progressbar" aria-label="Progresso da avaliação" aria-valuemin={0} aria-valuemax={100} aria-valuenow={progress}><span style={{ width: `${progress}%` }} /></div>
    ...
    <div className="pp-student-question-copy"><p>Pergunta {String(index + 1).padStart(2, "0")}</p><h1 id="assessment-question-title">{localText(question.label)}</h1>{question.description ? <span id={descriptionId}>{localText(question.description)}</span> : null}{question.required ? <small>Resposta obrigatória</small> : <small>Resposta opcional</small>}</div>
    <div className="pp-student-field"><QuestionField question={question} value={answers[question.key]} onChange={updateAnswer} labelledBy="assessment-question-title" describedBy={[descriptionId, errorId].filter(Boolean).join(" ") || undefined} /></div>
    {error ? <p id="assessment-question-error" className="matrix-message error" role="alert">{error}</p> : null}
